...
const am_hal_gpio_pincfg_t g_GPIO_FGPIO_INPUT =
{
    ...
    .GP.cfg_b.eDriveStrength       = AM_HAL_GPIO_PIN_DRIVESTRENGTH_0P1X,
    .GP.cfg_b.ePullup              = AM_HAL_GPIO_PIN_PULLUP_6K, //AM_HAL_GPIO_PIN_PULLUP_6K, //AM_HAL_GPIO_PIN_PULLDOWN_50K
		.GP.cfg_b.eGPRdZero = 				false,
};
...
//
//=============================================================================
void fgpio_write(uint32_t data)
{
		FPIO->WT0 = data;
}
...
uint32_t	wr_data=0XAAAA, rd_data;
    ...
    while (1)
    {
		...
		if(wr_data == 0XAAAA)wr_data=0X5555;
